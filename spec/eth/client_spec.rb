...
describe Client do
  ...
  subject(:geth_ws) { Client.create(geth_ws_path, { logger: logger }) }

  # public networks
  let(:drpc_api) { "https://eth.drpc.org" }
  subject(:drpc_mainnet) { Client.create drpc_api }
  let(:drpc_api_base) { "https://eth.drpc.org" }
  ...
  describe ".call" do
    ...
    it "calls a lendingpool contract on base returning tuple abi" do
      ...
      allow(drpc_base).to receive(:eth_call).and_return({ "result" => "0x" })
      allow_any_instance_of(Eth::Contract::Function).to receive(:decode_call_result).and_return([[[64, 0x04d5ddf5f3a8939889f11e97f8c4bb48317f1938, 0xcbafa036638f9b128bed110420d4594353d03c55, 0x5fcb1222e1986faa7f6b7b889ad756f9394c12cf, 366518821725046811550397, 31105542728196695647511, 1106784813611765991, 21216879519171931]]])
      result = drpc_base.call(lending_pool, "getReserveStatus", [64])
      expect(result).to eq [[64, 0x04d5ddf5f3a8939889f11e97f8c4bb48317f1938, 0xcbafa036638f9b128bed110420d4594353d03c55, 0x5fcb1222e1986faa7f6b7b889ad756f9394c12cf, 366518821725046811550397, 31105542728196695647511, 1106784813611765991, 21216879519171931]]
    end
  end
