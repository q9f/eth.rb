module Eth
  class Contract::Encoder
    def encode(type, value)
      is_array, arity, array_subtype = Eth::Contract::Abi.parse_array_type(type)
      if is_array && arity
        encode_static_array(arity, array_subtype, value)
      elsif is_array
        encode_dynamic_array(array_subtype, value)
      else
        core, subtype = Eth::Contract::Abi.parse_type(type)
    ...
    private
      ...
      def size_of_inputs(inputs)
        inputs.map do |input|
          _, arity, _ = Eth::Contract::Abi.parse_array_type(input.type)
